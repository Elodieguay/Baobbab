...
const ProtectedRoutes = ()  => {
    ...
    const findCurrentRoute = (pathname: string) => {
        return Object.values(AppRoutes).find((route) => {
           ...
           if (route.path?.toLowerCase() === pathname.toLowerCase()){
            return true
           } console.log(`Path mismatch: pathname=${pathname}, routePath=${route.path}`
           )
           return false;
    })
      };
    ...
    if (currentRoute?.accessMode === RouteAccessMode.Authenticated && !authToken) {
        return( 
    ...
    if (currentRoute?.accessMode === RouteAccessMode.Authenticated &&
        currentRoute.path === AppRoutes.Booking.path &&
        role!== UserRole.USER
    ) { 
        const redirectTo = AppRoutes.Booking.redirects as {authenticated: RouteNames; unauthenticated: RouteNames}
    ...
    if (currentRoute?.redirects) {
        const route = typeof currentRoute.redirects === 'object'
            ? authToken
